        ...
        <aside className="w-[280px] shrink-0 hidden lg:flex flex-col gap-8 sticky top-24 h-[calc(100vh-100px)] overflow-y-auto scrollbar-hide pr-2">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-gray-100 dark:border-gray-800">
                <h2 className="text-xl font-bold flex items-center gap-2">
                    Filters
                    <span className="text-xs font-normal text-text-muted bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-full">3 Active</span>
                </h2>
                    ...
                    {[
                        ...
                        <label key={i} className="flex items-center justify-between cursor-pointer group">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-sm text-text-main dark:text-gray-300 group-hover:text-primary transition-colors">{cat.name}</span>
                            </div>
                            <span className="text-xs text-text-muted bg-gray-50 dark:bg-gray-800 px-2 py-0.5 rounded-full group-hover:bg-primary/10 group-hover:text-primary transition-colors">{cat.count}</span>
                        </label>
                    ))}
                </div>
            </div>
            ...
            <div>
                ...
                <div className="px-1">
                    ...
                    <div className="flex items-center justify-between mt-4">
                        <span className="text-sm font-medium text-text-main dark:text-white">Range:</span>
            ...
            <div>
                ...
                <div className="flex flex-col gap-2">
                    {['Green House', 'Organic Farms', 'Fresh Local', 'Eco Market', 'Healthy Life'].map((brand, i) => (
                        <label key={i} className="flex items-center gap-3 cursor-pointer group">
                            ...
                            <span className="text-sm text-text-main dark:text-gray-300 group-hover:text-primary transition-colors">{brand}</span>
                        </label>
                    ))}
                </div>
            </div>
            ...
            <div className="mt-4 rounded-xl overflow-hidden relative h-80 group cursor-pointer shadow-lg">
                <div
                    ...
                    style={{ backgroundImage: "url('https://lh3.googleusercontent.com/aida-public/AB6AXuC4bDAHC4b7UhmkO7nHgs2vjXZ6egKyv4IZCUP0MkH7vjcgd1cC8QHldltnUEO6MlNr4dXHibNB4RQuCo6yRd7P1GtjRzHpiqXxVKzr_vsOBodY_qSyASHz4W-zg4urN9781S_pH7TTOTSe0DfITyYSOs-c0epBdkrJrH_kaI8c0zjROsQqnjofaE-yNE1MV59WEW-U_V5xVEiYH_3qXPlgQSaF81DN0HUciwkA93D_MS6XDSNodbtzRSssU-guwikE_6JG6xijPJMr')" }}
                ></div>
                <div className="absolute inset-0 bg-black/40"></div>
                <div className="absolute top-0 left-0 p-6 flex flex-col items-center text-center w-full h-full justify-center">
                    <span className="text-secondary font-bold tracking-widest text-sm mb-2">ORGANIC</span>
                    <h3 className="text-white font-bold text-2xl mb-4">Save 17% on <br />Breakfast</h3>
