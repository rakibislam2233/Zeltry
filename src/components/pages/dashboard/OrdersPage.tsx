            ...
            <div className="space-y-4">
                {filteredOrders.length > 0 ? (
                    filteredOrders.map(order => (
                        <div key={order.id} className="bg-white rounded border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow">
                            <div className="flex flex-col md:flex-row gap-6">
                                {/* Image */}
                                <div className="w-20 h-20 flex-shrink-0 bg-gray-50 rounded overflow-hidden flex items-center justify-center p-2">
                                    <img src={order.image} alt={order.product} className="w-full h-full object-contain mix-blend-multiply" />
                                </div>
                                ...
                                <div className="flex-1 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    <div className="space-y-2">
                                        <div className="flex items-center gap-3">
                                            <span className="font-bold text-text-main text-lg">Order {order.id}</span>
                                        ...
                                        <p className="text-sm font-medium text-text-main line-clamp-1">{order.product} <span className="text-text-muted font-normal">+ {order.items > 1 ? order.items - 1 : 0} more</span></p>
                                    </div>

                                    <div className="flex items-center justify-between md:flex-col md:items-end gap-2 md:gap-4 pl-0 md:pl-4 md:border-l border-gray-200 min-w-[140px]">
                                        <div className="hidden md:flex flex-col items-end">
                                            <span className="font-bold text-lg text-primary">${order.total.toFixed(2)}</span>
                                            <span className="text-xs text-text-muted">{order.items} Items</span>
                                        </div>
                                        <button className="flex items-center gap-2 px-4 py-2 rounded border border-gray-200 text-sm font-medium hover:bg-primary hover:text-white hover:border-primary transition-all w-full md:w-auto justify-center">
                    ...
                    <div className="text-center py-20 bg-white rounded border border-gray-200">
                        ...
                        <h3 className="text-lg font-bold text-text-main mb-2">No orders found</h3>
