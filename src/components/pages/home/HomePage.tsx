...
import { ArrowRight, Truck, Headphones, ShieldCheck, Package, Menu, Search, ShoppingCart } from "lucide-react";
                ...
                <div className="relative h-full min-h-[400px] md:min-h-[500px] rounded-lg overflow-hidden bg-[#F2F2F2] flex items-center p-8 md:p-12">
                     ...
                     <div className="relative z-10 space-y-6 max-w-sm">
                        <span className="text-primary font-bold tracking-wider text-sm uppercase flex items-center gap-2">
                             <span className="h-[1px] w-8 bg-primary"></span> Weekend Deal
                        </span>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 p-6 border rounded-lg bg-card shadow-sm">
             {[
                ...
                <div key={i} className="flex items-center gap-4">
                     <div className="h-12 w-12 flex-shrink-0 rounded-full bg-background flex items-center justify-center text-primary border">
      ...
      <section className="container mx-auto px-4 mt-16">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            ...
            <div className="flex items-center gap-4 text-sm font-medium text-muted-foreground">
                <span className="text-primary font-bold border-b-2 border-primary pb-0.5">All</span>
                <span className="hover:text-primary cursor-pointer transition-colors">Vegetables</span>
                <span className="hover:text-primary cursor-pointer transition-colors">Fruits</span>
                <span className="hover:text-primary cursor-pointer transition-colors">Meat & Fish</span>
            </div>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-0 border-t border-l">
            {/* Using 5 cols to match the dense grid often seen in these templates, applying border to create grid lines */}
            ...
            <div className="border-r border-b p-4"><ProductCard name="Green Capsicum" price={9.00} rating={5} image="/placeholder" category="Vegetables" isNew /></div>
            <div className="border-r border-b p-4"><ProductCard name="Chili" price={12.00} rating={4} image="/placeholder" category="Vegetables" /></div>
        </div>
      </section>
